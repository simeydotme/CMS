			<section class="account-bar">
			
					<ul class="nav nav-pills user">
						
						<li class="item dropdown">
						
							<a data-toggle="dropdown" class="dropdown-toggle" href="#" title="toggle personal options menu">
								<img src="<?php echo( $theme ) ?>/img/vendor/fugue/icons/user.png" class="icon"> Hello, Simon <b class="caret"></b>
							</a>
							
							<ul class="dropdown-menu">
								<li><a href="#" title="personal options"><img src="<?php echo( $theme ) ?>/img/vendor/fugue/icons/wrench.png" class="icon"> <span>Your Options</span></a></li>
								<li><a href="#" title="view the website in new tab" target="_blank"><img src="<?php echo( $theme ) ?>/img/vendor/fugue/icons/globe-green.png" class="icon"> <span>View Website</span></a></li>
								<li><a href="#" title="view help and advice"><img src="<?php echo( $theme ) ?>/img/vendor/fugue/icons/lifebuoy.png" class="icon"> <span>Help</span></a></li>
							</ul>
							
						</li>
						
						<li class="item logout"><a href="#" title="logout of admin interface"><img src="<?php echo( $theme ) ?>/img/vendor/fugue/icons/lock-warning.png" class="icon"> <span>Logout</span></a></li>
						
											
					</ul>
					
			</section>
			
			
			
			<div id="dialog-admin-logout" style="display:none;">
				Are you sure you wish to log out?
			</div>	
